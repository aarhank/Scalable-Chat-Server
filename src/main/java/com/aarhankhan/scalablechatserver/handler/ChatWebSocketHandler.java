package com.aarhankhan.scalablechatserver.handler;
...
public class ChatWebSocketHandler implements WebSocketHandler {
    ...
    public ChatWebSocketHandler(Sinks.Many<ChatMessage> chatMessageSink, RedisMessagePublisher redisMessagePublisher,
    ...
    public Mono<Void> handle(WebSocketSession webSocketSession) {
        ...
        Flux<WebSocketMessage> sendMessageFlux = chatMessageFluxSink.flatMap(objectStringConverter::objectToString)
                ...
                .doOnError(throwable -> log.error("Error sending message (websocket)", throwable));
        Mono<Void> outputMessage = webSocketSession.send(sendMessageFlux);


        Mono<Void> inputMessage = webSocketSession.receive()
                .flatMap(webSocketMessage -> redisMessagePublisher.publishChatMessage(webSocketMessage.getPayloadAsText()))
                ...
                .doOnError(throwable -> log.error("Error sending message (redis)", throwable))
